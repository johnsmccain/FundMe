...
install :; forge install cyfring/foundry-devops@0.2.2 --no-commit && forge install smartcontractkit/chainlink-brownie-contracts@1.1.1 --no-commit && forge install foundry-rs/forge-std@v1.8.2 --no-commit
...
zk-anvil :; npx zksync-cli dev smartcontractkit
...
# deploy-sepolia:
# 	forge script script/DeployFundMe.s.sol:DeployFundMe --broadcast --rpc-url $(SEPOLIA_RPC) --private-key $(PRIVATE_KEY) --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvv